				...
				<div class="my-cell" style="">
					<?php
						if (isset($_COOKIE['watchlist']) && (in_array($usRow['ClassId'], json_decode($_COOKIE['watchlist'])))) {
							#Means in bookmark
							?><button class="my-button w3-hover-green w3-hover-text-white" onclick="unbookmark('<?php echo $usRow['ClassId']; ?>')"><i class="fas fa-bookmark"></i></button><?php
						}
						else{
							?><button class="my-button w3-hover-green w3-hover-text-white" onclick="bookmark('<?php echo $usRow['ClassId']; ?>')"><i class="far fa-bookmark"></i></button><?php	
						}
					?>
				</div>
			</div><?php
