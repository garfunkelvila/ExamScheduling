...
<body>
		...
		<div id="view-1">
			...
			<form class="w3-card-4 w3-padding w3-container" onsubmit="return changeView('1','2');">
				...
				<div style="display: table;">
					<div style="display: table-cell;"><input class="my-input-1"  type="text" id="fName" placeholder="First name" required="" onkeyup="loginIdFiller()"></div>
					<div style="display: table-cell;"><input class="my-input-1" type="text" id="mName" placeholder="Middle name"></div>
					<div style="display: table-cell;"><input class="my-input-1" type="text" id="lName" placeholder="Family name" required="" onkeyup="loginIdFiller()"></div>
				</div>
	...
	<script type="text/javascript">
		...
		function updateSudgestion(){
			if ($("#txbGuardianId").val().length < 3){
				$("#sudgestions").html("");
				return;
			};

			$.ajax({
				url: "ajax_select_users_popup_register_guardian_pre_reg.php",
				dataType: "json",
				data: {
					q: $("#txbGuardianId").val(),
					idNum: tmpId
		...
		function fillSubjContainer(){
			$.ajax({
				url: "fragment_table_pre_reg_guardian_monitor.php",
				data: { idNum: tmpId },
				success: function(response){
		...
		function addStudent(q){
			$.ajax({
				url: "insert_pre_reg_guardian_monitor.php",
				data: {
					studentId: q,
					tUserId: tmpId
				},
		...
		function removeStudent(e){
			$.ajax({
				url: "ajax_delete_from_guardian_monitor_pre_reg.php",
				data: {
					monitorId: e
				},
				success: function(response){
